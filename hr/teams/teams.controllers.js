...
const { PrismaClient, TeamRole } = pkg;
			...
			await prisma.employeeTeam.createMany({
				...
				onConflict: {
					...
					action: {
						update: {
							role: (_, options) =>
								roles?.[options.index] ? roles[options.index] : TeamRole.MEMBER,
						},
					},
				},
			});
...
const addEmployeesToTeam = asyncHandler(async (req, res) => {
	...
	try {
		...
		const newEmployeeData = employeeData.filter(
			(data) => !existingEmployeeIds.includes(data.employeeId)
		);
		...
		if (newEmployeeData.length === 0) {
			return res
				.status(400)
				.json({ error: 'All employees are already in the team' });
		}
	...
	try {
		...
		if (missingEmployees.length > 0) {
			return res
				.status(400)
				.json({
					error: `Employees (${missingEmployees.join(
						', '
					)}) are not in the team`,
				});
		}
		...
		res
			.status(204)
			.json({ message: 'Employees removed from the team successfully' });
	} catch (error) {
		console.error(error);
		res.status(500).json({ error: error.message });
	}
